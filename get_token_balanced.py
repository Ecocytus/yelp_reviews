...
def update_dict(singe_review, rating) :
    lot = nltk.word_tokenize(singe_review, language='english')
    length = len(lot)
    for tokens in lot:
        ...
        (dict_review_token[tokens])[int(float(rating)/0.5-2)] += 1/length # make it fair between long and short reviews
...
for url in lourl:
    ...
    sum_len += print_restaurant_info(url,0,0)
    update_token_file()

print("number of review is: %d", sum_len)
